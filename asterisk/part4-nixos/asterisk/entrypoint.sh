#!/bin/bash
# Entrypoint для Asterisk (раздел 4):
# 1. Ждёт MySQL
# 2. Генерирует PKI-сертификаты (CA → Asterisk-сервер → клиенты)
# 3. Обновляет таблицу ps_endpoints в БД (TLS-транспорт + SRTP)
# 4. Запускает Asterisk

set -e

# Create log file immediately so fail2ban can start monitoring before Asterisk is up
...
for i in $(seq 1 60); do
    if mysqladmin -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASS" \
        ...
        echo "[entrypoint] MySQL is ready (attempt $i)"
        break
    fi
    [ "$i" -eq 60 ] && { echo "[entrypoint] ERROR: MySQL not available after 120s"; exit 1; }
    sleep 2
done

# ── 2. Генерация сертификатов ─────────────────────────────────────────────
if [ ! -f "$KEYS_DIR/ca.crt" ]; then
    echo "[entrypoint] Generating PKI certificates..."
    mkdir -p "$KEYS_DIR"
    cd "$KEYS_DIR"
    umask 077

    # 2a. Корневой CA (раздел 4.6)
    openssl genrsa -out ca.key 2048 2>/dev/null
    openssl req -x509 -new -nodes -key ca.key -sha256 -days 3650 \
        -subj "/C=RU/ST=Moscow/L=Zelenograd/O=MIET/OU=TCS/CN=My-CA" \
        -out ca.crt

    # 2b. Ключ и сертификат сервера Asterisk
...
EXTEOF
    ...
    # 2c. Клиентские сертификаты для каждого абонента (раздел 4.8)
    #     Подписаны сертификатом Asterisk-сервера, а не корневым CA
    for EXT in 4000 4001; do
        ...
        # PEM = cert + key (нужен PhonerLite)
        cat "client${EXT}.crt" "client${EXT}.key" > "client${EXT}.pem"
    done

    # Права доступа (раздел 4.8)
    chown -R asterisk:asterisk "$KEYS_DIR"
    chmod 600 "$KEYS_DIR"/*.key
    chmod 644 "$KEYS_DIR"/*.crt "$KEYS_DIR"/*.pem 2>/dev/null || true
    echo "[entrypoint] Certificates generated in $KEYS_DIR"
else
    echo "[entrypoint] Certificates already exist, skipping generation"
fi

# ── 3. Обновление БД: TLS-транспорт и SRTP для абонентов ─────────────────
echo "[entrypoint] Updating database endpoints for TLS/SRTP..."
...
WHERE id = '4001';
SQL
echo "[entrypoint] Database updated"
