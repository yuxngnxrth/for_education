#!/bin/bash
# Entrypoint для Nginx (раздел 4):
# 1. Создаёт PKI-сертификаты для HTTPS (раздел 4.6)
# 2. Создаёт файл .htpasswd для HTTP Basic Auth (раздел 4.7)
# 3. Запускает nginx
...
if [ ! -f "$SSL_DIR/pma.crt" ] || [ ! -f "$SSL_DIR/dhparam.pem" ]; then
    ...
    # Параметры DH для Forward Secrecy (раздел 4.6)
